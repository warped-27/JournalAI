...
interface NotesActions {
  ...
  createNote: (draft: Pick<Note, 'title' | 'content'> & { attachments?: Note['attachments'] }) => Promise<Note>;
...
export function createNotesStore(repo: NotesRepository) {
  return createStore<NotesStore>((set, get) => ({
    ...
    async createNote(draft) {
      const now  = Date.now();
      const note: Note = { id: newId(), title: draft.title, content: draft.content, attachments: draft.attachments ?? [], createdAt: now, updatedAt: now };
